"""Exponential accelerated failure time regression."""
...
@dataclass(slots=True)
class ExponentialAFTFittedResult:
    model: ExponentialAFTModelData
    params: pd.Series
    bse: pd.Series
    tvalues: pd.Series
    pvalues: pd.Series
    ...
    def predict(self, exog: np.ndarray | pd.DataFrame | None = None, *, kind: str = "median") -> np.ndarray:
        ...
        if kind == "linear":
            return eta
        if kind != "median":
            raise ValueError("Exponential AFT prediction kind must be 'median', 'mean', 'scale', or 'linear'.")
        return scale * float(np.log(2.0))
...
def _negative_log_likelihood(parameters: np.ndarray, duration: np.ndarray, event: np.ndarray, exog: np.ndarray) -> float:
    eta = exog @ parameters
    cumulative_hazard = duration * np.exp(-eta)
    loglike = event * (-eta) - cumulative_hazard
...
def _fit_exponential_parameters(
    duration: np.ndarray,
    event: np.ndarray,
    exog: np.ndarray,
    *,
...
def fit_exponential_aft(
    ...
    *,
    ...
    model_id: str = "exponential_aft_1",
    add_intercept: bool = True,
    maximum_iterations: int = 500,
) -> RegressionResult:
    """Fit an exponential accelerated failure time model for right-censored durations."""
    ...
    duration, event, predictors, metadata = _prepare_survival_design(
        ...
        model_label="Exponential AFT regression",
    )
    if add_intercept:
    ...
    fit = _fit_exponential_parameters(
        duration_values,
        event_values,
        exog,
        maximum_iterations=maximum_iterations,
    )
    parameter_count = exog.shape[1]
    covariance = _covariance_matrix(fit, parameter_count)
    beta = np.asarray(fit.x, dtype=float)
    standard_errors = np.sqrt(np.where(np.diag(covariance) >= 0, np.diag(covariance), np.nan))
    ...
    scale = np.exp(linear_predictor)
    median_time = scale * float(np.log(2.0))
    residuals = np.log(duration_values) - linear_predictor
    llf = -float(fit.fun)
    null_exog = np.ones((len(duration_values), 1))
    null_fit = _fit_exponential_parameters(
        duration_values,
        event_values,
        null_exog,
        maximum_iterations=maximum_iterations,
    ...
    if not fit.success:
        warnings.append("Exponential AFT optimizer did not report formal convergence.")

    fitted = ExponentialAFTFittedResult(
        model=ExponentialAFTModelData(
    ...
    return RegressionResult(
        ...
        fit_statistics={
            ...
            "mean_predicted_time": float(np.mean(scale)),
