...
def fit_regression_by_level(
    ...
    *,
    ...
    if model_type == "quasi_poisson":
        options = mixed_effects_options or {}
        return fit_quasi_poisson(
            dataframe,
            dependent_variable=dependent_variable,
            independent_variables=independent_variables,
            fixed_effects=fixed_effects,
            model_id=model_id,
            covariance_type=str(options.get("covariance_type", "nonrobust")),
    ...
    if model_type == "panel_between_effects":
        ...
        time_variable = str(options.get("time_variable", "")).strip() or None
        if not entity_variable:
            raise ValueError("Panel between effects requires entity_variable.")
        return fit_panel_between_effects(
            dataframe,
            dependent_variable=dependent_variable,
            independent_variables=independent_variables,
            entity_variable=entity_variable,
    ...
    if model_type == "weibull_ph":
        options = mixed_effects_options or {}
        event_variable = str(options.get("event_variable", "")).strip()
        if not event_variable:
            raise ValueError("Weibull PH regression requires event_variable.")
    ...
    if model_type == "stratified_cox":
        options = mixed_effects_options or {}
        event_variable = str(options.get("event_variable", "")).strip()
        strata_variable = str(options.get("strata_variable", options.get("strata", ""))).strip()
        if not event_variable:
            raise ValueError("Stratified Cox regression requires event_variable.")
        if not strata_variable:
            raise ValueError("Stratified Cox regression requires strata_variable.")
        return fit_stratified_cox(
            dataframe,
            duration_variable=dependent_variable,
            event_variable=event_variable,
