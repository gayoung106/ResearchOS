...
def fit_zero_inflated_poisson(
    ...
    *,
    ...
    model_id: str = "zero_inflated_poisson_1",
    covariance_type: str = "HC3",
    add_intercept: bool = True,
    maximum_iterations: int = 300,
) -> RegressionResult:
    """절편 전용 inflation 식을 사용하는 ZIP 모형을 적합한다."""
    ...
    if (outcome < 0).any():
        raise ValueError("영과잉 포아송 종속변수는 0 이상의 값이어야 합니다.")

    rounded_outcome = np.round(outcome)
    if not np.allclose(outcome, rounded_outcome):
        raise ValueError("영과잉 포아송 종속변수는 음이 아닌 정수로 코딩되어야 합니다.")
    ...
    if not converged:
        result_warnings.append("영과잉 포아송 모형이 수렴하지 않았습니다.")

    if optimization_warnings:
        result_warnings.append("영과잉 포아송 최적화 과정에서 수치 경고가 기록되었습니다.")
