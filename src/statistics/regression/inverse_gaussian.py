"""Inverse Gaussian GLM regression for positive continuous outcomes."""
...
def fit_inverse_gaussian_regression(
    dataframe: pd.DataFrame,
    *,
    dependent_variable: str,
    independent_variables: list[str],
    fixed_effects: list[str] | None = None,
    model_id: str = "inverse_gaussian_regression_1",
    covariance_type: str = "HC3",
    add_intercept: bool = True,
    maximum_iterations: int = 100,
) -> RegressionResult:
    """Fit an Inverse Gaussian GLM with log link for strictly positive outcomes."""
    ...
    design = prepare_regression_design_matrix(
        ...
        model_label="Inverse Gaussian regression",
    )
    outcome = design.outcome.astype(float)
    if (outcome <= 0.0).any():
        raise ValueError("Inverse Gaussian regression dependent variable must be strictly positive.")
