"""Diagnostics for Inverse Gaussian GLM regression."""
...
class InverseGaussianPredictionMetrics:
...
@dataclass(slots=True)
class InverseGaussianDiagnosticsReport:
    model_id: str
    model_type: str
    sample_size: int
    multicollinearity: list[MulticollinearityResult]
    prediction_metrics: InverseGaussianPredictionMetrics
    observations: pd.DataFrame
    warnings: list[str] = field(default_factory=list)
    summary: dict[str, Any] = field(default_factory=dict)


def _validate_inverse_gaussian_result(result: RegressionResult) -> Any:
    if result.model_type != "inverse_gaussian_regression":
        raise ValueError("Inverse Gaussian diagnostics require model_type='inverse_gaussian_regression'.")
    if result.raw_result is None:
        raise ValueError("A fitted statsmodels result is required for Inverse Gaussian diagnostics.")
    return result.raw_result


def calculate_inverse_gaussian_multicollinearity(
    result: RegressionResult,
) -> list[MulticollinearityResult]:
    fitted = _validate_inverse_gaussian_result(result)
...
def build_inverse_gaussian_diagnostics(result: RegressionResult) -> InverseGaussianDiagnosticsReport:
    fitted = _validate_inverse_gaussian_result(result)
    ...
    metrics = InverseGaussianPredictionMetrics(
    ...
    multicollinearity = calculate_inverse_gaussian_multicollinearity(result)
    warnings = [
    ...
    if dispersion is not None and float(dispersion) > 2.0:
        warnings.append("Inverse Gaussian dispersion ratio is above 2.0; model fit should be reviewed.")
    ...
    return InverseGaussianDiagnosticsReport(
        model_id=result.model_id,
        model_type=result.model_type,
        sample_size=result.sample_size,
...
def inverse_gaussian_multicollinearity_to_dataframe(
    report: InverseGaussianDiagnosticsReport,
) -> pd.DataFrame:
    return pd.DataFrame([asdict(item) for item in report.multicollinearity])


def inverse_gaussian_prediction_metrics_to_dataframe(
    report: InverseGaussianDiagnosticsReport,
) -> pd.DataFrame:
...
def inverse_gaussian_observations_to_dataframe(
    report: InverseGaussianDiagnosticsReport,
) -> pd.DataFrame:
    return report.observations.copy()


def inverse_gaussian_diagnostic_summary_to_dataframe(
    report: InverseGaussianDiagnosticsReport,
) -> pd.DataFrame:
