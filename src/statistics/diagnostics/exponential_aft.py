"""Diagnostics for exponential accelerated failure time models."""
...
class ExponentialAFTDiagnosticsReport:
    model_id: str
    model_type: str
    sample_size: int
    event_count: int
...
def _validate_exponential_aft_result(result: RegressionResult) -> Any:
    if result.model_type != "exponential_aft":
        raise ValueError("Exponential AFT diagnostics require model_type='exponential_aft'.")
    if result.raw_result is None:
        raise ValueError("A fitted Exponential AFT result is required.")
    return result.raw_result


def calculate_exponential_aft_multicollinearity(result: RegressionResult) -> list[MulticollinearityResult]:
    fitted = _validate_exponential_aft_result(result)
...
def build_exponential_aft_diagnostics(result: RegressionResult) -> ExponentialAFTDiagnosticsReport:
    fitted = _validate_exponential_aft_result(result)
    ...
    multicollinearity = calculate_exponential_aft_multicollinearity(result)
    ...
    if events_per_parameter < 10:
        warnings.append("Exponential AFT regression has fewer than 10 events per estimated coefficient.")
    ...
    return ExponentialAFTDiagnosticsReport(
        model_id=result.model_id,
        model_type=result.model_type,
        sample_size=result.sample_size,
        event_count=event_count,
...
def exponential_aft_multicollinearity_to_dataframe(report: ExponentialAFTDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame([asdict(item) for item in report.multicollinearity])


def exponential_aft_residuals_to_dataframe(report: ExponentialAFTDiagnosticsReport) -> pd.DataFrame:
    return report.residuals.copy()


def exponential_aft_prediction_metrics_to_dataframe(report: ExponentialAFTDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame(
        {"item": list(report.prediction_metrics.keys()), "value": list(report.prediction_metrics.values())}
    )


def exponential_aft_diagnostic_summary_to_dataframe(report: ExponentialAFTDiagnosticsReport) -> pd.DataFrame:
