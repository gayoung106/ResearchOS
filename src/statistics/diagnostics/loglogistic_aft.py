"""Diagnostics for log-logistic accelerated failure time models."""
...
class LogLogisticAFTDiagnosticsReport:
    model_id: str
    model_type: str
    sample_size: int
    event_count: int
...
def _validate_loglogistic_aft_result(result: RegressionResult) -> Any:
    if result.model_type != "loglogistic_aft":
        raise ValueError("Log-logistic AFT diagnostics require model_type='loglogistic_aft'.")
    if result.raw_result is None:
        raise ValueError("A fitted Log-logistic AFT result is required.")
    return result.raw_result


def calculate_loglogistic_aft_multicollinearity(result: RegressionResult) -> list[MulticollinearityResult]:
    fitted = _validate_loglogistic_aft_result(result)
...
def build_loglogistic_aft_diagnostics(result: RegressionResult) -> LogLogisticAFTDiagnosticsReport:
    fitted = _validate_loglogistic_aft_result(result)
    ...
    cox_snell = np.logaddexp(0.0, float(fitted.shape) * log_residual)
    ...
    multicollinearity = calculate_loglogistic_aft_multicollinearity(result)
    ...
    return LogLogisticAFTDiagnosticsReport(
        model_id=result.model_id,
        model_type=result.model_type,
        sample_size=result.sample_size,
        event_count=event_count,
...
def loglogistic_aft_multicollinearity_to_dataframe(report: LogLogisticAFTDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame([asdict(item) for item in report.multicollinearity])


def loglogistic_aft_residuals_to_dataframe(report: LogLogisticAFTDiagnosticsReport) -> pd.DataFrame:
    return report.residuals.copy()


def loglogistic_aft_prediction_metrics_to_dataframe(report: LogLogisticAFTDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame(
        {"item": list(report.prediction_metrics.keys()), "value": list(report.prediction_metrics.values())}
    )


def loglogistic_aft_diagnostic_summary_to_dataframe(report: LogLogisticAFTDiagnosticsReport) -> pd.DataFrame:
