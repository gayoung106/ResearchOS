"""Diagnostics for Tweedie GLM regression."""
...
class TweediePredictionMetrics:
    mean_absolute_error: float
    root_mean_squared_error: float
    mean_absolute_percentage_error_positive: float | None
...
@dataclass(slots=True)
class TweedieDiagnosticsReport:
    model_id: str
    model_type: str
    sample_size: int
    multicollinearity: list[MulticollinearityResult]
    prediction_metrics: TweediePredictionMetrics
    observations: pd.DataFrame
    warnings: list[str] = field(default_factory=list)
    summary: dict[str, Any] = field(default_factory=dict)


def _validate_tweedie_result(result: RegressionResult) -> Any:
    if result.model_type != "tweedie_regression":
        raise ValueError("Tweedie diagnostics require model_type='tweedie_regression'.")
    if result.raw_result is None:
        raise ValueError("A fitted statsmodels result is required for Tweedie diagnostics.")
    return result.raw_result


def calculate_tweedie_multicollinearity(result: RegressionResult) -> list[MulticollinearityResult]:
    fitted = _validate_tweedie_result(result)
...
def build_tweedie_diagnostics(result: RegressionResult) -> TweedieDiagnosticsReport:
    fitted = _validate_tweedie_result(result)
    ...
    positive = observed > 0
    ape = np.full_like(observed, np.nan, dtype=float)
    ape[positive] = np.abs(raw_residual[positive] / observed[positive])
    mape_positive = float(np.mean(ape[positive])) if np.any(positive) else None
    ...
    observations = pd.DataFrame(
        {
            ...
            "absolute_percentage_error_positive": ape,
        }
    )
    metrics = TweediePredictionMetrics(
        mean_absolute_error=float(np.mean(np.abs(raw_residual))),
        root_mean_squared_error=float(np.sqrt(np.mean(raw_residual**2))),
        mean_absolute_percentage_error_positive=mape_positive,
        ...
        observed_zero_proportion=float(np.mean(np.isclose(observed, 0.0))),
    )
    multicollinearity = calculate_tweedie_multicollinearity(result)
    warnings = [
    ...
    summary = {
        ...
        "mean_absolute_percentage_error_positive": metrics.mean_absolute_percentage_error_positive,
    ...
    return TweedieDiagnosticsReport(
        model_id=result.model_id,
        model_type=result.model_type,
        sample_size=result.sample_size,
...
def tweedie_multicollinearity_to_dataframe(report: TweedieDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame([asdict(item) for item in report.multicollinearity])


def tweedie_prediction_metrics_to_dataframe(report: TweedieDiagnosticsReport) -> pd.DataFrame:
    values = asdict(report.prediction_metrics)
    return pd.DataFrame({"item": list(values.keys()), "value": list(values.values())})


def tweedie_observations_to_dataframe(report: TweedieDiagnosticsReport) -> pd.DataFrame:
    return report.observations.copy()


def tweedie_diagnostic_summary_to_dataframe(report: TweedieDiagnosticsReport) -> pd.DataFrame:
