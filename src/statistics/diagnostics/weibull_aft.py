"""Diagnostics for Weibull accelerated failure time models."""
...
class WeibullAFTDiagnosticsReport:
    model_id: str
    model_type: str
    sample_size: int
    event_count: int
...
def _validate_weibull_aft_result(result: RegressionResult) -> Any:
    if result.model_type != "weibull_aft":
        raise ValueError("Weibull AFT diagnostics require model_type='weibull_aft'.")
    if result.raw_result is None:
        raise ValueError("A fitted Weibull AFT result is required.")
    return result.raw_result


def calculate_weibull_aft_multicollinearity(result: RegressionResult) -> list[MulticollinearityResult]:
    fitted = _validate_weibull_aft_result(result)
...
def build_weibull_aft_diagnostics(result: RegressionResult) -> WeibullAFTDiagnosticsReport:
    fitted = _validate_weibull_aft_result(result)
    ...
    cox_snell = (duration / np.asarray(fitted.predict(kind="scale"), dtype=float)) ** float(fitted.shape)
    ...
    multicollinearity = calculate_weibull_aft_multicollinearity(result)
    ...
    return WeibullAFTDiagnosticsReport(
        model_id=result.model_id,
        model_type=result.model_type,
        sample_size=result.sample_size,
        event_count=event_count,
...
def weibull_aft_multicollinearity_to_dataframe(report: WeibullAFTDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame([asdict(item) for item in report.multicollinearity])


def weibull_aft_residuals_to_dataframe(report: WeibullAFTDiagnosticsReport) -> pd.DataFrame:
    return report.residuals.copy()


def weibull_aft_prediction_metrics_to_dataframe(report: WeibullAFTDiagnosticsReport) -> pd.DataFrame:
    return pd.DataFrame(
        {"item": list(report.prediction_metrics.keys()), "value": list(report.prediction_metrics.values())}
    )


def weibull_aft_diagnostic_summary_to_dataframe(report: WeibullAFTDiagnosticsReport) -> pd.DataFrame:
