...
def _build_modified_poisson_effects(result: RegressionResult) -> EffectSizeReport:
    fitted = result.raw_result
    if fitted is None:
        raise ValueError("A fitted modified Poisson result is required.")
    ...
    for index, term in enumerate(exog_names):
        ...
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=float(np.exp(coefficient.confidence_interval_upper)),
                magnitude=None,
                interpretation="Risk ratio from a modified Poisson robust variance model.",
            )
        )
        effects.append(
            EffectSizeResult(
                term=term,
                effect_type="average_marginal_effect",
                estimate=float(np.mean(probabilities * params[index])),
                standard_error=None,
                statistic=coefficient.statistic,
                p_value=coefficient.p_value,
                confidence_interval_lower=None,
                confidence_interval_upper=None,
                magnitude=None,
                interpretation="Average marginal effect on event probability from a modified Poisson model.",
            )
        )
...
def _build_log_binomial_effects(result: RegressionResult) -> EffectSizeReport:
    fitted = result.raw_result
    if fitted is None:
        raise ValueError("A fitted log-binomial result is required.")
    ...
    for index, term in enumerate(exog_names):
        ...
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=float(np.exp(coefficient.confidence_interval_upper)),
                magnitude=None,
                interpretation="Risk ratio from a log-binomial model.",
            )
        )
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=None,
                magnitude=None,
                interpretation="Average marginal effect on event probability from a log-binomial model.",
            )
        )
...
def _build_exponential_aft_effects(result: RegressionResult) -> EffectSizeReport:
    effects: list[EffectSizeResult] = []
    for coefficient in result.coefficients:
        if coefficient.term.lower() in {"const", "intercept"}:
            continue
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=float(np.exp(coefficient.confidence_interval_upper)),
                magnitude=None,
                interpretation="Survival time ratio from an exponential AFT model.",
            )
        )
        effects.append(
            EffectSizeResult(
    ...
    return EffectSizeReport(
        ...
        model_effects={
            ...
            "events_per_parameter": result.fit_statistics.get("events_per_parameter"),
            "constant_hazard": result.fit_statistics.get("constant_hazard"),
        },
        metadata={
            "sample_size": result.sample_size,
            "duration_variable": result.metadata.get("duration_variable"),
    ...
    for coefficient in result.coefficients:
        ...
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=float(np.exp(coefficient.confidence_interval_upper)),
                magnitude=None,
                interpretation="Median survival time ratio from a log-normal AFT model.",
            )
        )
        effects.append(
            EffectSizeResult(
    ...
    for coefficient in result.coefficients:
        ...
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=float(np.exp(coefficient.confidence_interval_upper)),
                magnitude=None,
                interpretation="Time ratio from a Weibull accelerated failure time model.",
            )
        )
        effects.append(
            EffectSizeResult(
...
def _build_binary_cloglog_effects(result: RegressionResult) -> EffectSizeReport:
    fitted = result.raw_result
    if fitted is None:
        raise ValueError("A fitted binary cloglog result is required.")

    effects: list[EffectSizeResult] = []
    exog = np.asarray(fitted.model.exog, dtype=float)
    exog_names = [str(name) for name in fitted.model.exog_names]
    params = np.asarray(fitted.params, dtype=float)
    linear_predictor = exog @ params
    derivative = np.exp(linear_predictor - np.exp(linear_predictor))
    coefficient_lookup = {coefficient.term: coefficient for coefficient in result.coefficients}
    ...
    for index, term in enumerate(exog_names):
        ...
        effects.append(
            EffectSizeResult(
                term=term,
                effect_type="cloglog_hazard_ratio",
                estimate=coefficient.exponentiated_estimate,
                standard_error=None,
                statistic=coefficient.statistic,
                p_value=coefficient.p_value,
                ...
                confidence_interval_upper=float(np.exp(coefficient.confidence_interval_upper)),
                magnitude=None,
                interpretation="Exponentiated coefficient from a complementary log-log binary model.",
            )
        )
        effects.append(
            EffectSizeResult(
                ...
                confidence_interval_upper=None,
                magnitude=None,
                interpretation="Average marginal effect on event probability from a binary cloglog model.",
            )
        )
...
def build_regression_effect_size_report(
    ...
    if result.model_type == "binary_cloglog":
        return _build_binary_cloglog_effects(result)
