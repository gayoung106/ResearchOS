...
def write_korean_results_narrative(
    ...
    model_name = {
        ...
        "gee_inverse_gaussian": "GEE inverse Gaussian regression",
        "gee_tweedie": "GEE Tweedie regression",
        "poisson": "포아송 회귀분석",
    ...
    for coefficient in substantive:
        ...
        if regression_result.model_type in {"ols", "weighted_least_squares", "heckman_selection", "iv_2sls_regression", "regularized_regression", "robust_regression", "quantile_regression", "tobit_regression", "panel_fixed_effects", "panel_random_effects", "panel_between_effects", "mixed_random_intercept", "mixed_random_slope", "gee_gaussian"}:
            beta = (
                effect_lookup.get(
                    (
                        coefficient.term,
        ...
        elif regression_result.model_type in {"gamma_regression", "inverse_gaussian_regression", "tweedie_regression", "gee_gamma", "gee_inverse_gaussian", "gee_tweedie"}:
            ...
            risk_ratio = effect_lookup.get((coefficient.term, "risk_ratio"))
            effect_text = (
                f"RR={risk_ratio:.3f}"
    ...
    if regression_result.model_type == "ols":
    ...
    elif regression_result.model_type == "panel_between_effects":
        entity_variable = regression_result.metadata.get("entity_variable")
        time_variable = regression_result.metadata.get("time_variable")
        entity_count = regression_result.fit_statistics.get("entity_count")
        time_count = regression_result.fit_statistics.get("time_period_count")
        ...
        if entity_count is not None and entity_variable is not None:
            sentences.append(
                f"Panel between effects estimated entity-level mean differences for {int(entity_count)} entities defined by {entity_variable}."
            )
        if time_count is not None and time_variable is not None:
        ...
        if entity_count is not None and entity_variable is not None:
            sentences.append(
                f"Panel random effects estimated random intercepts for {int(entity_count)} entities defined by {entity_variable}."
            )
        if time_count is not None and time_variable is not None:
            sentences.append(f"The panel covered {int(time_count)} periods defined by {time_variable}.")
        ...
        if random_variance is not None:
            sentences.append(f"The random-intercept variance was {float(random_variance):.3f}.")
        ...
        if event_count is not None and censored_count is not None:
            sentences.append(
                f"The log-logistic AFT model included {int(event_count)} events and "
                f"{int(censored_count)} censored observations."
            )
        if event_variable is not None:
            sentences.append(f"Event status was defined by {event_variable}.")
        if shape is not None:
            sentences.append(f"The log-logistic shape parameter was {float(shape):.3f}.")
        if median_time is not None:
            sentences.append(f"Median predicted survival time was {float(median_time):.3f}.")
        if events_per_parameter is not None:
            sentences.append(f"Events per parameter was {float(events_per_parameter):.2f}.")
...
def build_regression_publication_report(
    ...
    if regression_result.model_type in {"ols", "weighted_least_squares", "heckman_selection", "iv_2sls_regression", "regularized_regression", "robust_regression", "quantile_regression", "tobit_regression", "panel_fixed_effects", "panel_random_effects", "panel_between_effects"}:
    ...
    if regression_result.model_type in {"gee_gaussian", "gee_logit", "gee_poisson", "gee_negative_binomial", "gee_gamma", "gee_inverse_gaussian", "gee_tweedie"}:
