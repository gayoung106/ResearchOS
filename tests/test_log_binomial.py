...
def _binary_data() -> pd.DataFrame:
    ...
    x = rng.normal(size=n)
    z = rng.normal(size=n)
    risk = np.exp(-1.35 + 0.35 * x - 0.25 * z)
    risk = np.clip(risk, 0.02, 0.82)
    y = rng.binomial(1, risk)
    return pd.DataFrame({"y": y, "x": x, "z": z})


def test_fit_log_binomial_integrates_reporting_visualization_and_audit(tmp_path: Path) -> None:
    data = _binary_data()
    result = fit_log_binomial(
        data,
    ...
    assert result.model_type == "log_binomial"
    ...
    assert "Log-binomial" in report.narrative
    assert any("Log-binomial models report" in note for note in report.notes)
    assert {Path(path).name for path in visual.output_files} == {"coefficient_forest.png"}
    assert audit.metadata["link"] == "log"


def test_selector_routes_explicit_log_binomial() -> None:
...
def test_log_binomial_diagnostics_and_pipeline_step(tmp_path: Path) -> None:
    data = _binary_data()
    result = fit_log_binomial(
        data,
        dependent_variable="y",
    ...
    step_result = RegressionDiagnosticsStep(runtime, model_id="main_model").run(
        ResearchContext(project_name="log binomial diagnostics"),
        tmp_path,
    )
    audit = build_research_audit_report(runtime, model_id="main_model")

    assert diagnostics.model_type == "log_binomial"
    ...
    assert runtime.get_artifact("regression_diagnostics:main_model").model_type == "log_binomial"
    assert any("Log-binomial diagnostics" in item.evidence for item in audit.items)


def test_builder_registers_explicit_log_binomial_pipeline(tmp_path: Path) -> None:
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="log binomial builder"),
    ...
    assert registration.model_type == "log_binomial"
