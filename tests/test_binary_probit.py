...
def _binary_data() -> pd.DataFrame:
    ...
    x = rng.normal(size=n)
    z = rng.normal(size=n)
    latent = -0.1 + 0.85 * x - 0.55 * z + rng.normal(0.0, 0.9, size=n)
    y = (latent > 0.0).astype(int)
    return pd.DataFrame({"y": y, "x": x, "z": z})


def test_fit_binary_probit_integrates_reporting_visualization_and_audit(tmp_path: Path) -> None:
    data = _binary_data()
    result = fit_binary_probit(
        data,
    ...
    assert result.model_type == "binary_probit"
    ...
    assert "Binary probit" in report.narrative
    assert any("Binary probit reports" in note for note in report.notes)
    assert {Path(path).name for path in visual.output_files} == {"coefficient_forest.png"}
    assert audit.metadata["link"] == "probit"


def test_selector_routes_explicit_binary_probit() -> None:
...
def test_binary_probit_diagnostics_and_pipeline_step(tmp_path: Path) -> None:
    data = _binary_data()
    result = fit_binary_probit(
        data,
        dependent_variable="y",
    ...
    step_result = RegressionDiagnosticsStep(runtime, model_id="main_model").run(
        ResearchContext(project_name="binary probit diagnostics"),
        tmp_path,
    )
    audit = build_research_audit_report(runtime, model_id="main_model")

    assert diagnostics.model_type == "binary_probit"
    ...
    assert runtime.get_artifact("regression_diagnostics:main_model").model_type == "binary_probit"
    assert any("Binary probit diagnostics" in item.evidence for item in audit.items)


def test_builder_registers_explicit_binary_probit_pipeline(tmp_path: Path) -> None:
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="binary probit builder"),
    ...
    assert registration.model_type == "binary_probit"
