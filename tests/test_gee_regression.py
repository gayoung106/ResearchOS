...
def _gamma_data() -> pd.DataFrame:
    ...
    x = rng.normal(size=len(clusters))
    cluster_effect = rng.normal(0.0, 0.25, size=7)
    mean = np.exp(0.45 + 0.35 * x + cluster_effect[clusters])
    shape = 4.0
    scale = mean / shape
...
def _tweedie_data() -> pd.DataFrame:
    rng = np.random.default_rng(20260926)
    clusters = np.repeat(np.arange(7), 10)
    x = rng.normal(size=len(clusters))
    cluster_effect = rng.normal(0.0, 0.25, size=7)
    mean = np.exp(0.25 + 0.35 * x + cluster_effect[clusters])
    positive = rng.gamma(shape=2.5, scale=mean / 2.5)
    zero_mask = rng.random(size=len(clusters)) < 0.2
    y = np.where(zero_mask, 0.0, positive)
    return pd.DataFrame({"y": y, "x": x, "cluster": clusters})
...
def test_selector_routes_gee_gamma_and_effects_are_mean_ratios() -> None:
    result = fit_regression_by_level(
        _gamma_data(),
        dependent_variable="y",
        independent_variables=["x"],
...
def test_selector_routes_gee_tweedie_and_effects_are_mean_ratios() -> None:
    result = fit_regression_by_level(
        _tweedie_data(),
        dependent_variable="y",
        independent_variables=["x"],
        ...
        group_variable="cluster",
        mixed_effects_options={"covariance_structure": "exchangeable", "tweedie_var_power": 1.5},
    )
    effects = build_regression_effect_size_report(result)

    assert result.model_type == "gee_tweedie"
    assert result.fit_statistics["cluster_count"] == 7
    assert result.fit_statistics["tweedie_var_power"] == 1.5
    assert result.fit_statistics["tweedie_scale"] > 0
    assert all(coefficient.exponentiated_estimate is not None for coefficient in result.coefficients)
    assert any(effect.effect_type == "mean_ratio" for effect in effects.effects)
...
def test_builder_registers_explicit_gee_gamma_pipeline(tmp_path: Path) -> None:
    plan = AnalysisPlan.model_validate(
        {
            "variables": {
                "dependent": ["y"],
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="gee gamma builder"),
        working_directory=tmp_path,
    )
    ...
    assert registration.registered is True
    assert registration.model_type == "gee_gamma"
    assert registration.group_variable == "cluster"
    assert registration.diagnostics_registered is True
    assert registration.effect_size_registered is True
    assert registration.reporting_registered is True
...
def test_gee_gamma_diagnostics_reporting_and_audit(tmp_path: Path) -> None:
    result = fit_gee(
        _gamma_data(),
        dependent_variable="y",
        independent_variables=["x"],
    ...
    report = build_regression_publication_report(result, effects)
    visual = build_regression_visualizations(result, output_directory=tmp_path)
    runtime = PipelineRuntime(dataframe=_gamma_data())
    runtime.set_artifact("regression_result:main_model", result)
    runtime.set_artifact("effect_size_report:main_model", effects)
    runtime.set_artifact("regression_publication_report:main_model", report)
    runtime.set_artifact("regression_visualization:main_model", visual)
    ...
    assert diagnostics.model_type == "gee_gamma"
    assert diagnostics.cluster_count == 7
    assert "GEE accounted for 7 clusters defined by cluster." in report.narrative
    assert any("GEE models are population-averaged" in note for note in report.notes)
    assert {Path(path).name for path in visual.output_files} == {"coefficient_forest.png"}
...
def test_builder_registers_explicit_gee_tweedie_pipeline(tmp_path: Path) -> None:
    plan = AnalysisPlan.model_validate(
        {
            "variables": {
                "dependent": ["y"],
            ...
            "analyses": {
                "regression": {
                    "enabled": True,
                    "options": {
                        "estimator": "gee_tweedie",
                        "covariance_structure": "exchangeable",
                        "tweedie_var_power": 1.5,
                    },
                },
                "robustness": {"enabled": False},
            },
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="gee tweedie builder"),
        working_directory=tmp_path,
    )
    ...
    assert registration.registered is True
    assert registration.model_type == "gee_tweedie"
    assert registration.group_variable == "cluster"
    assert registration.diagnostics_registered is True
    assert registration.effect_size_registered is True
    assert registration.reporting_registered is True
...
def test_gee_tweedie_diagnostics_reporting_and_audit(tmp_path: Path) -> None:
    result = fit_gee(
        _tweedie_data(),
        dependent_variable="y",
        independent_variables=["x"],
    ...
    report = build_regression_publication_report(result, effects)
    visual = build_regression_visualizations(result, output_directory=tmp_path)
    runtime = PipelineRuntime(dataframe=_tweedie_data())
    runtime.set_artifact("regression_result:main_model", result)
    runtime.set_artifact("effect_size_report:main_model", effects)
    runtime.set_artifact("regression_publication_report:main_model", report)
    runtime.set_artifact("regression_visualization:main_model", visual)
    ...
    assert diagnostics.model_type == "gee_tweedie"
    assert diagnostics.cluster_count == 7
    assert "GEE accounted for 7 clusters defined by cluster." in report.narrative
    assert any("GEE models are population-averaged" in note for note in report.notes)
    assert {Path(path).name for path in visual.output_files} == {"coefficient_forest.png"}
    assert audit.metadata["cluster_count"] == 7
    assert audit.metadata["tweedie_var_power"] == 1.5
    assert audit.metadata["tweedie_scale"] > 0
