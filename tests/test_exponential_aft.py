...
def _survival_data() -> pd.DataFrame:
    ...
    x = rng.normal(size=n)
    z = rng.normal(size=n)
    scale = np.exp(1.0 + 0.36 * x - 0.20 * z)
    event_time = rng.exponential(scale=scale)
    censor_time = rng.exponential(scale=4.8, size=n)
    event = (event_time <= censor_time).astype(int)
    duration = np.minimum(event_time, censor_time) + 0.02
    return pd.DataFrame({"duration": duration, "event": event, "x": x, "z": z})


def test_fit_exponential_aft_integrates_reporting_visualization_and_audit(tmp_path: Path) -> None:
    data = _survival_data()
    result = fit_exponential_aft(
        data,
    ...
    assert result.model_type == "exponential_aft"
    assert result.metadata["distribution"] == "exponential"
    assert result.fit_statistics["event_count"] > 20
    assert result.fit_statistics["constant_hazard"] is True
    assert any(effect.effect_type == "time_ratio" for effect in effects.effects)
    assert "exponential AFT" in report.narrative
    assert any("Exponential AFT models report time ratios" in note for note in report.notes)
    assert {Path(path).name for path in visual.output_files} == {
        "coefficient_forest.png",
        "exponential_aft_survival_curve.png",
    }
    assert audit.metadata["constant_hazard"] is True


def test_selector_routes_explicit_exponential_aft() -> None:
...
def test_exponential_aft_diagnostics_and_pipeline_step(tmp_path: Path) -> None:
    data = _survival_data()
    result = fit_exponential_aft(
        data,
        duration_variable="duration",
    ...
    diagnostics = build_exponential_aft_diagnostics(result)
    runtime = PipelineRuntime(dataframe=data)
    runtime.set_artifact("regression_result:main_model", result)
    step_result = RegressionDiagnosticsStep(runtime, model_id="main_model").run(
        ResearchContext(project_name="exponential aft diagnostics"),
        tmp_path,
    )
    audit = build_research_audit_report(runtime, model_id="main_model")

    assert diagnostics.model_type == "exponential_aft"
    assert exponential_aft_residuals_to_dataframe(diagnostics).shape[0] == result.sample_size
    assert "concordance_index" in set(exponential_aft_prediction_metrics_to_dataframe(diagnostics)["item"])
    assert "constant_hazard" in set(exponential_aft_diagnostic_summary_to_dataframe(diagnostics)["item"])
    ...
    assert any("Exponential AFT diagnostics" in item.evidence for item in audit.items)


def test_builder_registers_explicit_exponential_aft_pipeline(tmp_path: Path) -> None:
    plan = AnalysisPlan.model_validate(
        {
            ...
            "analyses": {
                "regression": {
                    "enabled": True,
                    "options": {"estimator": "exponential_aft", "event_variable": "event"},
                },
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="exponential aft builder"),
    ...
    assert registration.model_type == "exponential_aft"
