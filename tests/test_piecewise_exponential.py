...
def _survival_data() -> pd.DataFrame:
    ...
    x = rng.normal(size=n)
    z = rng.normal(size=n)
    hazard = np.exp(0.50 * x - 0.35 * z)
    event_time = rng.exponential(scale=1.0 / hazard)
    censor_time = rng.exponential(scale=1.9, size=n)
    event = (event_time <= censor_time).astype(int)
    duration = np.minimum(event_time, censor_time) + 0.01
    return pd.DataFrame({"duration": duration, "event": event, "x": x, "z": z})


def test_fit_piecewise_exponential_integrates_reporting_visualization_and_audit(tmp_path: Path) -> None:
    data = _survival_data()
    result = fit_piecewise_exponential_regression(
        data,
    ...
    assert result.model_type == "piecewise_exponential"
    ...
    assert "baseline hazard was estimated across 4 time intervals" in report.narrative
    assert report.metadata["interval_count"] == 4
    assert {Path(path).name for path in visual.output_files} == {
        "coefficient_forest.png",
        "piecewise_exponential_survival_curve.png",
    }
    assert audit.metadata["interval_count"] == 4
    assert audit.metadata["total_exposure"] == result.fit_statistics["total_exposure"]


def test_selector_routes_piecewise_exponential_model() -> None:
    result = fit_regression_by_level(
        ...
        mixed_effects_options={"event_variable": "event", "breakpoints": [0.4, 0.9, 1.6]},
    )
...
def test_piecewise_exponential_diagnostics_and_pipeline_step(tmp_path: Path) -> None:
    data = _survival_data()
    result = fit_piecewise_exponential_regression(
        data,
        duration_variable="duration",
    ...
    diagnostics = build_piecewise_exponential_diagnostics(result)
    runtime = PipelineRuntime(dataframe=data)
    runtime.set_artifact("regression_result:main_model", result)
    step_result = RegressionDiagnosticsStep(runtime, model_id="main_model").run(
        ResearchContext(project_name="piecewise exponential diagnostics"),
        tmp_path,
    )

    assert diagnostics.model_type == "piecewise_exponential"
    assert piecewise_interval_hazards_to_dataframe(diagnostics).shape[0] == 4
    assert piecewise_residuals_to_dataframe(diagnostics).shape[0] == result.fit_statistics["long_row_count"]
    assert "total_exposure" in set(piecewise_diagnostic_summary_to_dataframe(diagnostics)["item"])
...
def test_builder_registers_explicit_piecewise_exponential_pipeline(tmp_path: Path) -> None:
    plan = AnalysisPlan.model_validate(
        {
            ...
            "analyses": {
                "regression": {
                    "enabled": True,
                    "options": {
                        "estimator": "piecewise_exponential",
                        "event_variable": "event",
                        "breakpoints": [0.4, 0.9, 1.6],
                    },
                },
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="piecewise exponential builder"),
    ...
    assert registration.model_type == "piecewise_exponential"
