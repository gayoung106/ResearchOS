"""Quasi-Poisson regression integration tests."""
...
def _overdispersed_count_dataframe(*, seed: int = 331, size: int = 260) -> pd.DataFrame:
...
def test_fit_quasi_poisson_integrates_outputs(tmp_path: Path) -> None:
    data = _overdispersed_count_dataframe()
    result = fit_quasi_poisson(
        data,
    ...
    assert result.model_type == "quasi_poisson"
    assert result.fit_statistics["dispersion_ratio"] > 1.0
    assert np.isclose(result.fit_statistics["scale"], result.fit_statistics["dispersion_ratio"])
    assert diagnostics.summary["dispersion_ratio"] == result.fit_statistics["dispersion_ratio"]
    assert any(effect.effect_type == "incidence_rate_ratio" for effect in effects.effects)
    assert "Quasi-Poisson" in report.narrative
    assert any(path.endswith("count_observed_vs_predicted.png") for path in visual.output_files)
    assert audit.metadata["model_type"] == "quasi_poisson"
    assert audit.metadata["quasi_likelihood"] is True


def test_selector_routes_explicit_quasi_poisson() -> None:
    result = fit_regression_by_level(
        _overdispersed_count_dataframe(),
        dependent_variable="y",
        independent_variables=["x", "z"],
    ...
    assert result.metadata["quasi_likelihood"] is True


def test_builder_registers_explicit_quasi_poisson(tmp_path: Path) -> None:
    analysis_plan = AnalysisPlan.model_validate(
        {
            ...
            "analyses": {
                "regression": {
                    "enabled": True,
                    "options": {"model_type": "quasi_poisson"},
                },
    ...
    orchestrator, _, registration = build_regression_pipeline(
        ...
        project_name="quasi poisson builder",
    )

    assert registration.registered is True
    assert registration.model_type == "quasi_poisson"
