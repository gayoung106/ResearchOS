"""Modified Poisson regression integration tests."""
...
def _binary_data() -> pd.DataFrame:
    ...
    x = rng.normal(size=n)
    z = rng.normal(size=n)
    risk = np.exp(-1.05 + 0.35 * x - 0.2 * z)
    risk = np.clip(risk, 0.03, 0.78)
    y = rng.binomial(1, risk)
    return pd.DataFrame({"y": y, "x": x, "z": z})


def test_fit_modified_poisson_integrates_reporting_visualization_and_audit(
    tmp_path: Path,
) -> None:
    data = _binary_data()
    ...
    assert result.model_type == "modified_poisson"
    assert result.metadata["link"] == "log"
    assert result.metadata["modified_poisson"] is True
    assert result.fit_statistics["event_count"] > 10
    assert result.fit_statistics["non_event_count"] > 10
    assert any(effect.effect_type == "risk_ratio" for effect in effects.effects)
    assert any(effect.effect_type == "average_marginal_effect" for effect in effects.effects)
    assert "Modified Poisson" in report.narrative
    assert any("Modified Poisson models report" in note for note in report.notes)
    assert {Path(path).name for path in visual.output_files} == {"coefficient_forest.png"}
    assert audit.metadata["modified_poisson"] is True


def test_selector_routes_explicit_modified_poisson() -> None:
...
def test_modified_poisson_diagnostics_and_pipeline_step(tmp_path: Path) -> None:
    data = _binary_data()
    result = fit_modified_poisson(
        data,
        dependent_variable="y",
    ...
    step_result = RegressionDiagnosticsStep(runtime, model_id="main_model").run(
        ResearchContext(project_name="modified poisson diagnostics"),
        tmp_path,
    )

    assert diagnostics.model_type == "modified_poisson"
    ...
    assert runtime.get_artifact("regression_diagnostics:main_model").model_type == "modified_poisson"


def test_builder_registers_explicit_modified_poisson_pipeline(tmp_path: Path) -> None:
    plan = AnalysisPlan.model_validate(
        {
            ...
            "analyses": {
                "regression": {
                    "enabled": True,
                    "options": {"estimator": "modified_poisson"},
                },
    ...
    orchestrator = ResearchOrchestrator(
        context=ResearchContext(project_name="modified poisson builder"),
    ...
    assert registration.model_type == "modified_poisson"
